...
def test_raw_mode_returns_unadjusted():
    ...
    result = read_bars_adjusted(con, ["sec_test"], datetime(2025, 6, 15, tzinfo=timezone.utc), price_mode="raw")
...
def test_adjustment_respects_pit_boundary():
    ...
    split_ts = datetime(2025, 6, 5, tzinfo=timezone.utc)
    split_data = splits_from_json(
        [{"date": "2025-06-01", "splitRatio": "2:1"}],
    ...
    as_of_before = datetime(2025, 6, 3, tzinfo=timezone.utc)
    ...
    as_of_after = datetime(2025, 6, 15, tzinfo=timezone.utc)

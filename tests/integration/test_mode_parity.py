...
def test_mode_parity_storage():
    ...
    df = pl.DataFrame({
        "security_id": ["sec_parity"], "effective_date": [date(2026, 6, 18)],
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        embedded_cfg = RootConfig(lake=LakeConfig(
            runtime="embedded",
        ...
        embedded_bars = read_bars_asof(
            con_emb, ["sec_parity"], datetime(2026, 6, 18, 17, 0, tzinfo=UTC),
        )
        con_emb.close()

    # --- 2. Stack mode ---
    with tempfile.TemporaryDirectory() as tmpdir:
        stack_cfg = RootConfig(lake=LakeConfig(
            runtime="stack",
            catalog=("ducklake:postgres:dbname=lake_catalog "
                     "host=postgres port=5432 user=lake password=lake"),
        ...
        stack_bars = read_bars_asof(
            con_stk, ["sec_parity"], datetime(2026, 6, 18, 17, 0, tzinfo=UTC),
        )
        con_stk.close()

        # Verify raw blob is NOT on local disk (would indicate split-brain)
        assert not Path(tmpdir).glob("**/*.zst"), \
            "stack mode: raw blob should NOT be on local disk"

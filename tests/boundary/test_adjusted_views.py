...
def test_raw_price_unaffected_by_future_adjustment(con):
    ...
    result = read_bars_adjusted(
        con, ["sec_lk"], datetime(2026, 1, 8, 12, 0, tzinfo=UTC), price_mode="split_adjusted"
    )
...
def test_adjustment_applied_when_knowable(con):
    ...
    result = read_bars_adjusted(
        con, ["sec_lk"], datetime(2026, 1, 10, 12, 0, tzinfo=UTC), price_mode="split_adjusted"
    )
...
def test_multiple_adjustment_sources_respect_visibility(con):
    ...
    s2 = splits_from_json(
        [{"date": "2026-01-15", "splitRatio": "3:1"}],
        "sec_lk",
        "eodhd_splits",
        "f2",
        "r1",
    ...
    early = read_bars_adjusted(
        con, ["sec_lk"], datetime(2026, 1, 10, 12, 0, tzinfo=UTC), price_mode="split_adjusted"
    )
    assert early["close"][0] == 50.0, "only first split visible"

    late = read_bars_adjusted(
        con, ["sec_lk"], datetime(2026, 2, 1, 12, 0, tzinfo=UTC), price_mode="split_adjusted"
    )

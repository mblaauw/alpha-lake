...
async def fetch_news(tickers: list[str], start_date: str, end_date: str) -> RawFetch:
    ...
    params: dict[str, Any] = {
        "token": cfg.api_key, "startDate": start_date, "endDate": end_date,
        "tickers": ",".join(tickers), "sort": "date",
    }
    async with build_client(cfg) as client:
        endpoint = "/tiingo/news"
        response = await fetch_with_retry(client, endpoint, params=params)
        manifest = build_manifest("tiingo", endpoint, params, response.content, response.status_code, 1)

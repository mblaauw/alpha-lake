...
**Alpha Vantage multi-dataset integration:** AV provides a free-tier (25 calls/day) single-API-key source covering fundamentals, corporate actions, insider transactions, institutional holdings, economic indicators, commodities, top movers, ETF profiles, IPO calendar, and listing status. The connector at `src/alpha_lake/connectors/alphav.py` batches related API calls (up to 7 per symbol for fundamentals) with 12-second delays to stay within rate limits. AV is configured as a secondary/fallback source via `[precedence.fundamentals]` to minimize API consumption. See ADR-0030 and `docs/adr/2026-06-25-alpha-vantage-integration.md` for full details.

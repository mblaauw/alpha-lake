...
**Decision:**
Use SCD2 (Slowly Changing Dimension Type 2) on the `available_at` (knowledge time) column. Every write creates a new version with a new `available_at`. The `effective_date` identifies the logical row, and `available_at` versions it. Queries use `available_at <= as_of` to get the correct version.

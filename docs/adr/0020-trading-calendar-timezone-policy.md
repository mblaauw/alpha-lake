...
- All instants are stored UTC: `available_at`, `ingested_at`, `validated_at`, source publish instants, and DuckLake commit/system time.
...
- Calendar version is part of replay/config metadata so historical fixture expectations remain deterministic across calendar library updates.
